...
this repository or existing code, inspect those sources and treat what you find
as evidence. Ask only for decisions that cannot be resolved locally.
...
Keep agent changes limited to the requested work, and do not add unrelated cleanup,
generated artifacts, or release/version changes unless the task requires them.
...
Do not attempt to publish releases, push release tags, upload packages,
or alter release metadata unless explicitly asked to do so by a maintainer.
